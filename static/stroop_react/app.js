...
class WordGenerator extends React.Component {
  ...
  generateWord() {
    ...
    const colorParity = Math.random() >= 0.5; // needs tweaking

    let color = this.randomFromArray(colors);
    let text;
    ...
    if (colorParity) {
      ...
      // different
      text = this.randomFromArray(colors.filter(e => e != color));
    }
  ...
  instantiateRecognition() {
    ...
    recognition.onerror = (event) => {
      ...
      console.error('recognition onerror: ' + error);

      if (event.error == 'not-allowed') this.setState({ errorCode: 'not-allowed' });
      else this.setState({ errorCode: 'error' });
    ...
    recognition.onnomatch = (event) => {
      ...
      this.setState({ errorCode: 'nomatch' });
    }
  ...
  initialize() {
    ...
    this.setState({ 
      interval: setInterval(async () => {
        ...
        // stop previous question recognition first
  ...
  async componentDidMount() {
    await this.instantiateRecognition();
    this.initialize();
  }

  render() {
    if (this.state.errorCode == 'error') {
      return (
    ...
    else if (this.state.errorCode == 'not-allowed') {
      return (
        <div className="container h-100 d-flex alert-window">
          <div className="alert alert-danger jumbotron my-auto w-100">
            <h1 className="display-3">Error</h1>
            <p className="lead">Please enable your microphone for the voice recognition to work.</p>
            <button type="button" class="close" dataDismiss="alert" ariaLabel="Close" onClick={this.initialize}>
              <span ariaHidden="true">&times;</span>&nbsp;Continue
            </button>
          </div>
        </div>
      )
    ...
    else if (this.state.errorCode == 'nomatch') {
      return (
        <div className="container h-100 d-flex alert-window">
          <div className="alert alert-danger jumbotron my-auto w-100">
            <h1 className="display-3">Error</h1>
            <p className="lead">Sorry, we can't seem to understand. Please ensure that there is no background noise.</p>
            <button type="button" class="close" dataDismiss="alert" ariaLabel="Close" onClick={this.initialize}>
              <span ariaHidden="true">&times;</span>&nbsp;Restart
            </button>
          </div>
        </div>
      )
...
class WordGeneratorKeyboard extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      timeout: null, // next question automatically loads
      color: null,
      text: null,
      totalCount: 0,
      count: {
        compatible: 0, // color and text same
        incompatible: 0
      },
      results: {
        compatible: 0,
  ...
  generateWord() {
    ...
    this.setState(state => ({
      color: color,
      text: text,
      currentParity: colorParity,
      count: {
        compatible: state.count.compatible + colorParity ? 1 : 0,
        incompatible: state.count.incompatible + colorParity ? 0 : 1 // not really efficient way of updating but I suppose it's a minor thing
      },
  ...
  handleDocumentKeydown(e) {
    ...
    // record end time first for most accurate timestamp
    let endTime = performance.now()

    let key = e.key;
    const keysPossible = ['r', 'g', 'y', 'b'];
    ...
    if (keysPossible.includes(key)) {
      ...
      let colorInput = colorMap[key];

      let deltaTime = endTime - this.state.startTime; // TODO: placeholder
      console.log(deltaTime);

      if (colorInput == this.state.color) {
        // correct

        if (this.state.currentParity) {
          // same color and text
          this.setState(state => ({
            results: {
              ...state.results, // keep all other key-value pairs
              compatible: state.results.compatible + 1
            },
            time: {
              ...state.time,
              compatible: [...state.time.compatible, deltaTime] // append deltaTime to pre-existing array
            },
            status: 'Correct'
          }));
        }
          ...
          this.setState(state => ({
            results: {
              ...state.results, // keep all other key-value pairs
              incompatible: state.results.incompatible + 1
            },
            time: {
              ...state.time,
              incompatible: [...state.time.incompatible, deltaTime]
            },
            status: 'Correct'
          }));
        }
        ...
        // number of incorrect can be calculated from the total - correct
        // therefore only need to store delta times for incorrect
        if (this.state.currentParity) {
          this.setState(state => ({
            time: {
              ...state.time,
              compatibleFalse: [...state.time.compatibleFalse, deltaTime]
            },
            status: 'Incorrect'
          }));
        }
          ...
          this.setState(state => ({
            time: {
              ...state.time,
              incompatibleFalse: [...state.time.incompatibleFalse, deltaTime]
            },
            status: 'Incorrect'
          }));
        }
      ...
      e.preventDefault();
      if (confirm('Quit test?')) {
        this.props.restartOnError();
      }
    }
  ...
  render() {
    if (this.state.endTest) {
      ...
      let timeDifference = Math.round(averageIncompatibleTime - averageCompatibleTime);

      console.log(averageCompatibleTime, averageIncompatibleTime, timeDifference)

      averageCompatibleTime = Math.round(averageCompatibleTime);
      averageIncompatibleTime = Math.round(averageIncompatibleTime);

      let displayedValues = {
        'Compatible Time': averageCompatibleTime,
      ...
      for (const [key, value] of Object.entries(displayedValues)) {
        ...
        rows.push(<TableRow key={key} label={key} value={value} />);
      }
        ...
        <div className="container results-box">
          <div className="card">
            <div className="card-body">
              ...
              {rows}
              <hr />
              <a href="/" className="btn btn-primary">End Test</a>
            </div>
          </div>
        </div>
      );
...
const TableRow = props => {
    ...
    return (
      // <li className="list-group-item">
        <h5><span className="font-bold">{props.label}: </span>{props.value}</h5>
      // </li>
...
const Status = props => {
  return (
    <h4 className="answer-status">{props.text}</h4>
  );
}
...
class Main extends React.Component {
  constructor(props) {
    ...
    // bindings
    this.handleToggleKeyboard = this.handleToggleKeyboard.bind(this);
      ...
      <div className="container h-100 d-flex alert-window">
        <div className="splash text-center jumbotron">
          ...
          <div className="information">
            <p>Say the <em>color</em> of the word after each word pops up.</p>
            <p>
